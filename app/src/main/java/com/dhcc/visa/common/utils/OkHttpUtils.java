...
import com.dhcc.visa.common.HttpInfo;
...
public class OkHttpUtils {
    ...
    public void upLoadFile(String key, String secret, File file) {
        ...
        try {
            ...
            if (response.isSuccessful()) {
                ...
                Log.e(":isSuccessful", jsonObject.toJSONString());
            } else {
                throw new Exception("upload onError code " + response);
            }
        } catch (IOException e) {
    ...
    public void doPostJsonAsync(JSONObject json, Callback callback) {
        RequestBody requestBody = RequestBody.create(MediaType.parse("application/json; charset=utf-8"), json.toString());
        Request request = new Request.Builder().url(Api.URL_BASE).post(requestBody).build();
        ...
        if (method == Method.POST) {
            ...
            if (null != info.getParams() && !info.getParams().isEmpty()) {
                ...
                for (String key : map.keySet()) {
                    builder.add(key, map.get(key) == null ? "" : map.get(key));
                    log.append(key + " =" + map.get(key) == null ? "" : map.get(key) + ", ");
                }
    ...
    public Interceptor CACHE_CONTROL_INTERCEPTOR = new Interceptor() {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            switch (builder.cacheType) {
                ...
                case CacheType.CACHE_THEN_NETWORK:
                    if (!NetworkUtils.CheckNetworkState(VisaApplication.getApplication())) {
                        request = request.newBuilder().cacheControl(CacheControl.FORCE_CACHE).build();
                    }
                    break;
            }
    ...
    private OkHttpUtils(Builder builder) {
    ...
    public static final class Builder {
        ...
        private File cachedDir = VisaApplication.getApplication().getApplicationContext().getExternalCacheDir();
